export function IndustryStrip() {
  const industries = ["🏠 Home Services","💆 Med Spa & Wellness","✂️ Salons & Barbershops","🔧 Contractors","🍽️ Restaurants & Food","🏥 Healthcare & Clinics","🛒 Retail","⚖️ Professional Services","🚗 Auto Services","+ More"];
  return (
...
export function HowItWorks() {
  const steps = [
    { num: "01", icon: "🎯", title: "Diagnose", text: "We audit your current digital presence — website, Google profile, competitors — and pinpoint exactly where you're losing leads and visibility." },
    { num: "02", icon: "🔧", title: "Build", text: "We set up your growth engine: GBP optimization, NAP consistency, lead capture, automated follow-up, newsletters, and ad campaigns — all in one coordinated push." },
    { num: "03", icon: "📊", title: "Grow", text: "We execute, optimize, and report every month. You see exactly what's working, what we're improving, and how every dollar is performing." },
  ];
  return (
    <section id="how-it-works" style={{ padding: "96px 0", background: "#fff" }}>
      <div style={{ maxWidth: 1140, margin: "0 auto", padding: "0 24px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 28 }} className="steps-grid-responsive">
          {steps.map((s) => (
            <div key={s.num} style={{ padding: "36px 28px", border: "1px solid var(--border)", borderRadius: 10, background: "var(--cream)", position: "relative", overflow: "hidden" }}>
...
export function WhyUs() {
  const points = [
    { icon: "🤖", title: "AI-Powered. Human-Led.", text: "We use cutting-edge AI tools to work faster and smarter — so you get results that used to require a team three times our size." },
    { icon: "📊", title: "No Vanity Metrics.", text: "We report on leads, calls, form fills, and booked appointments. Not impressions. You always know exactly what your investment is producing." },
    { icon: "🗓️", title: "Month-to-Month. No Traps.", text: "No long-term contracts. No fine print. We keep clients by delivering results — not by locking them in." },
    { icon: "⚡", title: "Fast Onboarding.", text: "Most clients are live within two weeks. We move fast, communicate clearly, and don't waste your time with unnecessary back-and-forth." },
  ];
  return (
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 80, alignItems: "center" }} className="why-grid-responsive">
          <div>
            ...
            {points.map((p) => (
              <div key={p.title} style={{ display: "flex", gap: 18, marginBottom: 28, paddingBottom: 28, borderBottom: "1px solid var(--border)" }}>
                <div style={{ width: 44, height: 44, borderRadius: 8, background: "var(--orange-dim)", border: "1px solid rgba(232,98,26,0.2)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18, flexShrink: 0 }}>{p.icon}</div>
                <div>
          ...
          <div style={{ background: "var(--navy)", borderRadius: 12, padding: 36, position: "relative" }}>
            ...
            <div style={{ fontSize: 18, fontWeight: 300, fontStyle: "italic", color: "rgba(255,255,255,0.8)", lineHeight: 1.65, marginBottom: 28 }}>
              "Most small businesses are <strong style={{ color: "#fff", fontStyle: "normal", fontWeight: 700 }}>invisible online</strong> — not because their product isn't good, but because nobody built them a system to get found. That's the gap we exist to close."
            </div>
