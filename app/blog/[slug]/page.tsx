...
/** Render paragraph text, turning each `links` entry (matched in order) into an internal link. */
function renderText(s: PostSection) {
  const text = s.text ?? "";
  if (!s.links?.length) return text;
  ...
  s.links.forEach((l, i) => {
    ...
    out.push(
      <Link key={i} href={l.href} style={{ color: "var(--orange)", fontWeight: 600, textDecoration: "underline", textUnderlineOffset: 3 }}>
        {l.text}
      </Link>
    );
...
export default async function BlogPost({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <>
      <JsonLd data={[articleSchema, breadcrumbSchema(crumbs), ...(post.faqs.length ? [faqSchema(post.faqs)] : [])]} />
      <main>
        <PageHero
        ...
        <article style={{ padding: "64px 0 24px", background: "#fff" }}>
          <div style={{ maxWidth: 760, margin: "0 auto", padding: "0 24px" }}>
            ...
            {post.sections.map((s, i) => {
              ...
              return (
                <p key={i} style={{ fontSize: 16.5, color: "var(--muted)", lineHeight: 1.8, marginBottom: 20 }}>{renderText(s)}</p>
              );
            })}
          </div>
        </article>
        ...
        <section style={{ padding: "64px 0 80px", background: "var(--cream)" }}>
          <div style={{ maxWidth: 760, margin: "0 auto", padding: "0 24px" }}>
            ...
            {/* Related links */}
            <div style={post.faqs.length > 0 ? { marginTop: 48, paddingTop: 32, borderTop: "1px solid var(--border)" } : undefined}>
